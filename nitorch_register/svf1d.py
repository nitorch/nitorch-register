...
import torch
from nitorch_interpol import pull, add_identity_grid
from nitorch_core.finite_differences import diff1d
...
def _pull_vel(vel, disp=None, dim=-1, ndim=None, *args, **kwargs):
    ...
    vel = vel.movedim(dim-1, -1).unsqueeze(-1)
    disp = disp.movedim(dim-1, -1).unsqueeze(-1)
    disp = add_identity_grid(disp)
    vel = pull(vel, disp, *args, **kwargs)
    vel = vel.squeeze(-1).movedim(-1, dim-1)
    return vel
