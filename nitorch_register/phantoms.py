...
def square(shape=(64, 64), radius=None, **backend):
    """Generate a square"""
    square = identity_grid(shape, **backend)
...
def squircle(shape=(64, 64), order=4, radius=None, **backend):
    """Generate a squircle"""
    squircle = identity_grid(shape, **backend)
...
def letterc(shape=(64, 64), radius=None, width=None, rotation=0, **backend):
    ...
    backend = dict(dtype=c.dtype, device=c.device)
    ...
    if rotation:
        mat = affine.affine_matrix_classic(dim=2, rotations=[rotation*3.14/180])
        mat = mat.to(**backend)
        aff = affine.affine_default(shape, **backend)
        mat = affine.affine_matmul(mat, aff)
        aff = affine.affine_inv(aff)
        mat = affine.affine_matmul(aff, mat)
        grid = affine_grid(mat, shape)
        c = pull(c.unsqueeze(-1), grid).squeeze(-1)
        c = (c > 0.5).to(**backend)
    return c
