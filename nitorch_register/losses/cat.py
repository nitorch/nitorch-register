...
def cat(moving, fixed, dim=None, acceleration=0, grad=True, hess=True,
    ...
    """
    fixed, moving = extra.to_max_backend(fixed, moving)
    if mask is not None:
        mask = mask.to(fixed.device)
    dim = dim or (fixed.dim() - 1)
    nc = moving.shape[-dim-1]                               # nb classes - bck
    fixed = extra.slice_tensor(fixed, slice(nc), -dim-1)    # remove bkg class
    nvox = py.prod(fixed.shape[-dim:])

    # log likelihood
    ll = moving*fixed
    ll -= fastmath.logsumexp(moving, dim=-dim-1, implicit=True)  # implicit lse
    if mask is not None:
        ll = ll.mul_(mask)
    ll = ll.sum().neg() / nvox
    out = [ll]
    ...
    if grad or (hess and acceleration > 0):
        # implicit softmax
        moving = fastmath.softmax(moving, dim=-dim-1, implicit=True)
    ...
    if hess:
        ...
        if acceleration > 0:
            h = allocate_h()
            h_diag = extra.slice_tensor(h, slice(nc), -dim-1)
            h_diag.copy_(moving*(1 - moving))
            # off diagonal elements
            c = 0
            for i in range(nc):
                pi = extra.slice_tensor(moving, i, -dim-1)
                for j in range(i+1, nc):
                    pj = extra.slice_tensor(moving, j, -dim-1)
                    out = extra.slice_tensor(h, nc+c, -dim-1)
...
def cat_nolog(moving, fixed, dim=None, grad=True, hess=True, mask=None):
    ...
    # log likelihood
    moving = moving.clamp_min(1e-5).div(
        moving.sum(-dim-1, keepdim=True).add_(1e-5).clamp_min_(1-1e-5)
    )
    fixed = fixed.clamp_min(0).div(
        fixed.sum(-dim-1, keepdim=True).clamp_min_(1)
    )
    ...
    if hess:
        ...
        diag = extra.slice_tensor(h, range(nc), -dim-1)
...
class Cat(OptimizationLoss):
    ...
    def __init__(self, log=False, acceleration=0, dim=None):
        ...
        log : bool, default=False
            Whether the input are logits (pre-softmax) or probits
            (post-softmax)
...
class AutoCat(AutoGradLoss):
    ...
    def __init__(self, log=False, acceleration=0, dim=None):
        cat = Cat(log, acceleration, dim)
        super().__init__(cat.loss)
