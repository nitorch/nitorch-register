...
from .utils_local import local_mean, prod as script_prod
import torch
...
def init_gmm2(x, y, bins=6, dim=None, mask=None):
    ...
    xmean = extra.quantile(x[..., 0], quantiles, dim=range(-dim, 0), keepdim=True, mask=mask)
    ymean = extra.quantile(y[..., 0], quantiles, dim=range(-dim, 0), keepdim=True, mask=mask)
...
def fit_lgmm2(x, y, bins=6, max_iter=20, dim=None,
    ...
    if theta is None:
        ...
        z = gmmfit.pop('resp')
        del gmmfit
        z = extra.fast_movedim(z, -1, -dim-1)  # [nmom(=1), k, *spatial]
