...
def dice(moving, fixed, dim=None, grad=True, hess=True, mask=None,
    ...
    if weighted is True:
        ...
        weighted = torch.as_tensor(weighted, **extra.backend(moving))
    ...
    if grad or (hess and fisher < 1):
        ...
        if grad:
            if weighted is not None:
                g *= weighted
            if add_background:
                g_last = extra.slice_tensor(g, slice(-1, None), -2)
                g = extra.slice_tensor(g, slice(-1), -2)
                g -= g_last
            if mask is not None:
                g *= mask
    ...
    if hess:
        ...
        if weighted is not None:
            h *= weighted
        if add_background:
            h_foreground = extra.slice_tensor(h, slice(-1), -2)
            h = extra.slice_tensor(h, slice(-1, None), -2)  # h background
            hshape = list(h.shape)
            hshape[-2] = nc*(nc+1)//2
            h = h.expand(hshape).clone()
            diag = extra.slice_tensor(h, range(nc), -2)
            diag += h_foreground
        if mask is not None:
            h *= mask
