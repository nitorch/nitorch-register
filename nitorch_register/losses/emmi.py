...
def emmi_prepare(moving, fixed, weights, prior, dim):
    ...
    batch = extra.expanded_shape(*shapes)
...
def emmi_hard(moving, fixed, dim=None, prior=None, fwhm=None, max_iter=32,
    ...
    *batch, J, K = prior.shape
    ...
    for n_iter in range(max_iter):
        ...
        #   p(z[n] == k | x[n] == j[n]) ∝ p(x[n] == j[n] | z[n] == k) *
        #                                 p(z[n] == k)
        #
        # . j[n] is the discretized fixed image
        # . p(z[n] == k) is the moving template
        # . p(x[n] == j[n] | z[n] == k) is the conditional prior
        #   evaluated at (j[n], k)
        # --------------------------------------------------------------
        ...
        # => H_jk = p(x == j, z == k) ∝ Σ_n p(z[n] == k | x[n] == j) *
        #                                   𝛿(x[n] == j)
        # --------------------------------------------------------------
        ...
        if fwhm:
            ...
            prior = smooth(prior, dim=1, basis=0, fwhm=fwhm, bound='replicate')
            prior = prior.transpose(-1, -2)

        # prior /= prior.sum(dim=[-1, -2], keepdim=True)
        # MI-like normalization
        prior /= add_tiny_(prior.sum(dim=-1, keepdim=True) *
                           prior.sum(dim=-2, keepdim=True), Nb)
    ...
    # ------------------------------------------------------------------
    # compute gradients
    # Keeping only terms that depend on y, the mutual information is
    # H[y]-H[x,y]
    ...
    if grad or hess:
        ...
        norm = fastmath.dot(g.transpose(-1, -2), moving.transpose(-1, -2))
...
def emmi_soft(moving, fixed, dim=None, prior=None, fwhm=None, max_iter=32,
    ...
    *batch, J, K = prior.shape
    ...
    for n_iter in range(max_iter):
        ...
        z += moving.log()
        z, ll = fastmath.softmax_lse(z, -2, lse=True, weights=weights)
    ...
    #      = Σ_nj \sum_j q(x[n] == j) log \sum_k p(x[n] == j | z[n] == k, h) *
    #                                            p(z[n] == k)
    if grad or hess:

        norm = fastmath.dot(prior.transpose(-1, -2).unsqueeze(-1),
...
def sample_prior_soft(prior, fixed, out=None):
    ...
    out = fastmath.dot(prior, fixed, out=out)
    return out
...
def scatter_prior_soft(prior, fixed, z):
    ...
    prior = fastmath.dot(z, fixed, out=prior)
    return prior
...
class EMMI(OptimizationLoss):
    ...
    def loss_grad_hess(self, moving, fixed, **kwargs):
        ...
        mask = kwargs.pop('mask', None)
        prior = getattr(self, '_prior', None) if self.cache else None
        ll, g, h, prior = emmi(moving, fixed, grad=True, hess=True,
                               weights=mask, dim=dim, fwhm=fwhm,
                               max_iter=max_iter, prior=prior,
                               return_prior=True, **kwargs)
        if self.cache:
            self._prior = prior
        return ll, g, h
