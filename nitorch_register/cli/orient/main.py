...
from nitorch_register.affine import (
    ...
    affine_to_layout, volume_layout, volume_layout_to_name,
    affine_matvec, affine_make_rect, affine_modify, affine_rmdiv
)
import torch
import os
...
def orient(inp, affine=None, layout=None, voxel_size=None, center=None,
    ...
    if voxel_size in (None, 'like') or len(voxel_size) == 0:
        voxel_size = get_voxel_size(aff_like)
    elif voxel_size == 'self':
        voxel_size = get_voxel_size(aff0)
    elif voxel_size == 'standard':
        voxel_size = 1.
    voxel_size = make_vector(voxel_size, dim)

    if not layout or layout == 'like':
        layout = affine_to_layout(aff_like)
    elif layout == 'self':
        layout = affine_to_layout(aff0)
    ...
    if center in (None, 'like') or len(center) == 0:
        ...
        center = affine_matvec(aff_like, center)
    elif center == 'self':
        center = (torch.as_tensor(shape, dtype=torch.float) - 1) * 0.5
        center = affine_matvec(aff0, center)
    elif center == 'standard':
        center = 0.
    center = make_vector(center, dim)
    ...
    if affine.numel() == dim*(dim+1):
        affine = affine_make_rect(affine.reshape(dim, dim+1))
    ...
    affine = affine_modify(affine, shape, voxel_size=voxel_size,
    ...
    if output_transform:
        transform = affine_rmdiv(affine, aff0)
