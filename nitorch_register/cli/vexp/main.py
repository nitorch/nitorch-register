...
from nitorch_register.affine import affine_default, affine_matvec
from nitorch_register.svf import exp as exp_velocity
import torch
import os
...
def vexp(inp, type='displacement', unit='voxel', inverse=False,
    ...
    if is_file:
        ...
        if torch.is_tensor(inp):
            inp = (inp.clone(), affine_default(shape=inp.shape[:3]))
    dat, aff = inp
    ...
    dat = exp_velocity(dat[None], inverse=inverse, steps=steps, bound=bound,
