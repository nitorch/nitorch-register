...
from nitorch_cli.parser import (
    next_isvalue, next_istag, check_next_isvalue, ParseError,
)
...
def parse_transform(args, options):
    ...
    opt.file, *args = args
    if isinstance(opt, struct.Velocity) and next_isvalue(args):
        opt.json, *args = args
        ...
        # Parse remaining top-level tags
            ...
            try:
                ...
                if options.interpolation[0] != 'l':
                    raise ParseError('interpolation should be an integer of "l"')
