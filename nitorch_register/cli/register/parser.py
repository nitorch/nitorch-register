from nitorch_cli.parser import (
    CommandParser, Actions, Validations, Positional, NamedGroup, Option, Group
)
...
parser = CommandParser('register', help=help1, add_help=False)
...
loss_choices = Positional('name', nargs='?', default='mi',
                              validation=Validations.choice(loss_choices),
                              convert=lambda x: loss_aliases.get(x, x),
                              help='Name of the image loss')
loss = NamedGroup('loss', loss_choices, '@loss', n='+',
...
loss.add_option('slicewise', ('-z', '--slicewise'), nargs='?', default=False,
                action=Actions.store_value(-1), convert=int,
                help='Make the loss slice-wise')
# conditional options
weight_option = Option('weight', ('-w', '--weight'), nargs='1',
                           default=None, convert=number_or_str(float),
                           help='Weight (= precision) or the loss.')
joint_option = Option('joint', ('-j', '--joint'), nargs='0',
                          default=False, action=Actions.store_true,
                          help='Joint weight map across channels.')
patch_option = Option('patch', ('-p', '--patch'), nargs='1*', default=10,
                          convert=number_or_str(float), help='Patch size')
stride_option = Option('stride', ('-s', '--stride'), nargs='1*', default=1,
                           convert=int, help='Strides between patches.')
cluster_option = Option('bins', ('-b', '--bins'), nargs=1, default=6,
                            convert=int, help='Number of bins in the mixture')
kernel_option = Option('kernel', ('-k', '--kernel'), nargs=1, default='g',
                           help='Kernel type: [gauss], square')
iter_option = Option('max_iter', ('-n', '--max-iter'), nargs=1, default=128,
                         convert=int, help='Maximum number of EM iterations')
loss.add_suboption('mi', 'norm', ('-m', '--norm'),
                   validation=Validations.choice(['studholme', 'arithmetic', 'geometric', 'no']),
...
loss.add_suboption('dice', 'weight', ('-w', '--weight'), nargs='*', default=False,
                   convert=float, action=Actions.store_true,
...
# fix/mov subgroups
file = Group('file', n=1, help='Volume to register')
...
file.add_option('resliced', ('-r', '--resliced'), nargs='?',
                ...
                action=Actions.store_value('{dir}{sep}{base}.resliced{ext}'),
...
file.add_option('fwhm', ('-f', '--fwhm'), nargs='0*',
                ...
                action=Actions.store_value([1, 'vox']),
...
file.add_option('bound', ('-b', '--bound'), nargs=1, default='dct2',
                validation=Validations.choice(['dct2', 'dct1', 'dft', 'dst2', 'dst1', 'zero', 'replicate']),
...
file.add_option('missing', '--no-missing', nargs=0,
                action=Actions.store_value([]), help='No missing values')
file.add_option('mind', '--mind', nargs='*2', default=[], convert=float,
                action=Actions.store_value([1, 2]))
fix = Group('fix', '@@fix', n=1)
fix.copy_from(file)
mov = Group('mov', '@@mov', n=1)
mov.copy_from(file)
loss.add_group(fix)
loss.add_group(mov)
...
optim_choices = Positional('name', nargs='?', default='unset',
                               validation=Validations.choice(optim_choices),
                               convert=lambda x: optim_aliases.get(x, x),
                               help='Name of the optimizer')
optim = NamedGroup('optim', optim_choices, '@@optim', n='?')
...
optim.add_option('crit', ('-c', '--crit'), nargs=1,
                 default='diff', validation=Validations.choice(['gain', 'diff']),
...
optim.add_suboption('gn', 'solver', ('-q', '--solver'), nargs=1,
                    default='cg', validation=Validations.choice(['cg', 'relax']),
                    help='Linear solver')
...
optim.add_suboption('nes', 'output', ('-o', '--output'), nargs='?',
                    default='', action=Actions.store_value('{dir}{sep}momentum.{iter}.nii.gz'),
...
optim.add_suboption('cg', 'beta', ('-b', '--beta'), nargs=1, default='pr',
                    validation=Validations.choice(cg_choices),
...
affine_choices = Positional('name', nargs='?', default='rigid',
                                validation=Validations.choice(affine_choices),
                                convert=lambda x: affine_aliases.get(x, x),
                                help='Name of the affine transform')
affine = NamedGroup('affine', affine_choices, '@affine', n='?', make_default=False)
affine.add_positional('factor', nargs='?', default=1., convert=float,
                      help='Penalty factor')
affine.add_option('position', ('-p', '--position'), default='sym', nargs=1,
                  validation=Validations.choice(['sym', 'mov', 'fix']),
...
nonlin_choices = Positional('name', nargs='?', default='svf',
                                validation=Validations.choice(nonlin_choices),
                                convert=lambda x: nonlin_aliases.get(x, x),
                                help='Name of the nonlinear transform')
nonlin = NamedGroup('nonlin', nonlin_choices, '@nonlin', n='?', make_default=False)
...
joptim_choices = Positional('name', nargs='?', default='interleaved',
                                validation=Validations.choice(joptim_choices),
                                convert=lambda x: joptim_aliases.get(x, x),
                                help='Joint optimization strategy ')
joptim = NamedGroup('optim', joptim_choices, '@optim', n='?')
...
pyr_choices = Positional('name', nargs='?', default='gaussian',
                             validation=Validations.choice(pyr_choices),
                             convert=lambda x: pyr_aliases.get(x, x),
                             help='Pyramid strategy ')
pyr = NamedGroup('pyramid', pyr_choices, '@pyramid', n='?')
