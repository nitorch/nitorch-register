...
from nitorch_interpol import resize_flow
from nitorch_register.shoot import greens
from nitorch_register.affine import affine_basis, affine_resize
from .optim import OGM as _OGM
from .regularisers import (
    absolute_grid, membrane_grid, bending_grid, lame_div, lame_shear
)
...
class GridLoss(TransformationLoss):
    ...
    def greens(self, shape, **backend):
        opt = dict(absolute=self.absolute,
                   ...
                   lame=self.lame,
                   factor=self.factor)
        kernel = greens(shape, **opt, **backend)
        return kernel
    ...
    def free(self):
        ...
        if not hasattr(self, 'optdat'):
            ...
            *self.pyramid, pre_level = self.pyramid
            ...
            old_shape = self.dat.shape[:-1]
            new_shape = [s*(2**factor) for s in old_shape]
            self.dat = resize_flow(self.dat, shape=new_shape)
            self.affine = affine_resize(self.affine, old_shape, factor)
            self.optdat = torch.nn.Parameter(self.dat, requires_grad=True)
            self.dat = self.optdat
...
class Linear(Transformation):
    ...
    def free(self):
        ...
        if nb_prm == 0:
            ...
            self.basis = affine_basis('T', self.dim)
        elif nb_prm == nb_t:
            ...
            self.basis = affine_basis('SE', self.dim)
            ...
            self.basis = affine_basis('CSO', self.dim)
            ...
            self.optdat = torch.nn.Parameter(self.dat, requires_grad=True)
            self.dat = self.optdat
            self.basis = affine_basis('Aff+', self.dim)
