...
from nitorch_fastmath import expm
from nitorch_interpol import affine_grid, pull, identity_grid_like
from nitorch_register.affine import (
    mean_space, affine_conv, affine_resize, affine_matmul, affine_lmdiv,
    affine_matvec, affine_inv, voxel_size as get_voxel_size)
from nitorch_register.svf import exp as vexp
from nitorch_register.shoot import greens_apply
from nitorch_register.flow_inv import flow_inv
from .parser import parse, ParseError, help
from .helpers import ffd_exp, samespace, BacktrackingLineSearch
from . import struct
...
def _autoreg(argv=None):
    ...
    for loss in options.losses:
        ...
        for f, m in zip(loss.fixed.dat, loss.moving.dat):
            print(f'   -| {list(m[0].shape)}, {get_voxel_size(m[1]).tolist()}')
            print(f'   -> {list(f[0].shape)}, {get_voxel_size(f[1]).tolist()}')
    print('Transforms')
    for trf in options.transformations:
        print(f' - {trf.name}')
        if isinstance(trf, struct.NonLinear):
            ...
            for pyramid in reversed(trf.pyramid):
                ...
                vx = get_voxel_size(trf.affine) / factor
...
def load_data(s):
    ...
    def load(files, is_label=False):
        ...
        for file in files:
            ...
            dat = extra.movedim(dat, -1, 0)
            dim = dat.dim() - 1
            qt = quantile(dat, (0.01, 0.95), dim=range(-dim, 0), keepdim=True)
            mn, mx = qt.unbind(-1)
    ...
    def compute_grad(dat):
        ...
        med = extra.unsqueeze(med, -1, 3)
        dat /= 0.5*med
        dat = diff(dat, dim=[1, 2, 3]).square().sum(-1)
        return dat
    ...
    for trf in s.transformations:
        ...
        if isinstance(trf, struct.Linear):
            ...
            if trf.shift:
                ...
                trf.shift = -affine_matvec(affinef, shift)
            else:
                trf.shift = 0.
        else:
        ...
        for trf in options.transformations:
            ...
            if isinstance(trf, struct.FFD):
                ...
                if not trf.smalldef:
                    ...
                    for loss1 in trf.losses:
                        loss += loss1.call(d)
                d = vexp(d[None], displacement=True)[0]
                if trf.smalldef:
...
def write_transforms(options):
    ...
    if nonlin:
        ...
        if isinstance(nonlin, struct.FFD):
            ...
            affine, _ = affine_resize(affine, shape, factor)
    ...
    if inv:
        ...
        if nonlin['disp'] is not None:
            ...
            if samespace(mov2nlin, nonlin['disp'].shape[:-1], moving.shape):
                g = identity_grid_like(nonlin['disp'].to(device))
            else:
                g = affine_grid(mov2nlin, moving.shape)
                g += pull(nonlin['disp'].to(device), g)
            # param to moving
...
def write_data(options):
    ...
    for trf in options.transformations:
        if isinstance(trf, struct.FFD):
            ...
            d = trf.dat.to(**backend)
            if need_inv:
                id = vexp(d[None], displacement=True, inverse=True)[0]
            d = vexp(d[None], displacement=True)[0]
            d_aff = trf.affine.to(**backend)
            break
