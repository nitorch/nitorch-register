...
from nitorch_interpol import resize, resize_flow, resize_affine
...
def write_data(files, options):
    ...
    for file, ofile in zip(files, ofiles):
        ...
        input_vx = get_voxel_size(file.affine)
        if options.voxel_size:
            if options.factor:
                raise ValueError('Cannot use both factor and voxel size')
            factor = input_vx / make_vector(options.voxel_size, 3)
        elif options.factor:
            factor = make_vector(options.factor, 3)
        elif options.shape:
            input_shape = make_vector(dat.shape[:-1], 3, dtype=torch.float32)
            output_shape = make_vector(options.shape, 3, dtype=torch.float32)
        ...
        if options.grid:
            old_shape = dat.shape[:-1]
            dat = resize_flow(dat, factor, output_shape, **opt)[0]
            affine = resize_affine(file.affine, old_shape, factor, output_shape, options.anchor)
        else:
            old_shape = dat.shape[:-1]
            dat = torch.movedim(dat, -1, 0)
            dat = resize(dat, factor, output_shape, **opt)
            affine = resize_affine(file.affine, old_shape, factor, output_shape, options.anchor)
            dat = torch.movedim(dat, 0, -1)
