...
from nitorch_core.finite_differences import rmind
from nitorch_register.affine import affine_default, affine_parameters
from nitorch_register import losses, optim as opt
from .preproc import soft_quantize_image, discretize_image, preproc_image
from .objects import (
    AffineModel,
    Affine2dModel,
    NonLinModel,
...
def make_image(dat, mask=None, affine=None,
    ...
    if affine is None:
        affine = affine_default(dat.shape[1:])
    ...
    if mind:
        ...
        for level in image:
            ...
            level.dat = rmind(level.dat, dim=dim, radius=int(radius),
                              fwhm=fwhm, bound=level.bound)
            level.dat = level.dat.movedim(-1, 0)
...
def make_affine(basis='rigid', position='symmetric', penalty=None, init=None):
    ...
    if init is not None:
        ...
        if init.dim() > 1:
            init, _ = affine_parameters(init, basis)
    return AffineModel(basis, position=position, penalty=penalty,
                               dat=init)
...
def make_affine_2d(plane, affine_ref, basis='rigid', position='symmetric',
    ...
    if basis is True:
        basis = 'rigid'
    return Affine2dModel(basis, plane=plane, ref_affine=affine_ref,
...
def make_nonlin(shape_or_init=None, model='svf', affine=None, voxel_size=None,
    ...
    if isinstance(shape_or_init, io.MappedArray):
        vel = Displacement(shape_or_init.fdata(device=device),
                           affine=shape_or_init.affine)

    else:
        shape: list = shape_or_init

        do_mean_space = isinstance(shape[0], Image)
        ...
        # build mean space
        if do_mean_space:
            space = MeanSpace(shape,
                                      voxel_size=voxel_size, vx_unit=vx_unit,
                                      pad=pad, pad_unit=pad_unit)
        else:
            space = Space(shape, affine, voxel_size)

        # allocate data
        vel = Displacement(space.shape, affine=space.affine, device=device)

    # build model
    Model: type = NonLinModel.subclass(model)
...
def make_nonlin_2d(plane, affine_ref, *args, **kwargs):
    ...
    if not nonlin:
        return None
    nonlin = Nonlin2dModel(nonlin, plane, affine_ref)
    return nonlin
...
def make_nonlin_optim(optim=None, order=None, ls='wolfe', lr=None,
    ...
    if optim == 'gd':
        ...
        nonlin_optim = opt.FlowCG if solver == 'cg' else opt.FlowRelax
    elif optim == 'lbfgs':
