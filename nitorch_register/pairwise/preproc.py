...
from nitorch_core.extra import make_vector, backend
from nitorch_core import dtypes, padding
from ..affine import affine_default, affine_lmdiv, affine_pad, voxel_size
...
def preproc_image(input, mask=None, label=False, missing=0,
    ...
    if not torch.is_tensor(input):
        ...
        affine0 = affine_default(dat.shape[1:])
    dim = dat.dim() - 1
    ...
    for transform in affine:
        ...
        affine0 = affine_lmdiv(transform.to(affine0), affine0)
    ...
    if pad:
        ...
        if unit == 'mm':
            ...
            pad = torch.as_tensor(pad, **backend(vx))
            pad = pad / vx
        ...
        if any(pad):
            affine0, _ = affine_pad(affine0, dat.shape[-dim:], pad, side='both')
            dat = padding.pad(dat, pad, side='both', mode=bound)
            if mask0 is not None:
                mask0 = padding.pad(mask0, pad, side='both', mode=bound)
    ...
    if fwhm:
        ...
        if unit == 'mm':
            vx = voxel_size(affine)
            fwhm = torch.as_tensor(fwhm, **backend(vx))
            fwhm = fwhm / vx
        dat = smooth(dat, dim=dim, fwhm=fwhm, bound=bound)
...
def prepare_pyramid_levels(images, levels, dim=None, **opt):
    ...
    return pyramid_levels(vxs, shapes, levels, **opt)
...
def load_image(input, dim=None, device=None, label=False, missing=0):
    ...
    if not torch.is_tensor(input):
        ...
        dat, affine = input, affine_default(input.shape[1:])
    if label:
        dtype = dat.dtype
...
def rescale_image(dat, quantiles=0.95):
    ...
    """
    dim = dat.dim() - 1
    quantiles = make_vector(quantiles).tolist()
    if len(quantiles) == 0:
        mn = 0
        mx = 95
        ...
        mn, mx = quantiles
    mx = mx / 100
    mn, mx = quantile(dat, (mn, mx), dim=range(-dim, 0),
...
def discretize_image(dat, nbins=256):
    ...
    """
    dim = dat.dim() - 1
    mn, mx = quantile(dat, (0.0005, 0.9995), dim=range(-dim, 0), keepdim=True).unbind(-1)
...
def soft_quantize_image(dat, nbins=16):
    ...
    centers = torch.linspace(0, nbins, nbins+1, **backend(dat))
