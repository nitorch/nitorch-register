...
"""
from nitorch_fastmath import matvec, lmdiv, sym_matmul, batchmatvec
from nitorch_core.patches import unfold
from nitorch_core.hist import quantile
from nitorch_core.broadcast import expanded_shape
from nitorch_interpol import add_identity_grid, affine_grid, flow_jacobian
import torch
from .. import (
    optim as optm,
    utils as regutils,
...
def _almost_identity(aff):
    eye = torch.eye(aff.shape[-1], dtype=aff.ftype, device=aff.device)
...
class PairwiseRegisterStep:
    ...
    def mov2fix(self, fixed, moving, warped, vel=None, title=None, lam=None):
        ...
        for f, w in zip(fixed, warped):
            ...
            broad_shape = expanded_shape(f.shape, w.shape)
            f = f.expand(broad_shape).clone()
            w = w.expand(broad_shape)
            checker_unfold = unfold(f, patch, [2*p for p in patch])
            warped_unfold = unfold(w, patch, [2*p for p in patch])
    ...
    def do_vel(self, vel, grad=False, hess=False, in_line_search=False):
        ...
        for loss in self.losses:
            ...
            if _almost_identity(aff_right) and fixed.shape == self.nonlin.shape:
                aff_right = None
                phi = add_identity_grid(phi00)
                disp = phi00
            else:
                right = affine_grid(aff_right, fixed.shape)
            ...
            if _almost_identity(aff_left) and moving.shape == self.nonlin.shape:
                ...
                phi = affutils.affine_matvec(aff_left, phi)
            ...
            if do_print:
                has_printed = True
                disp = matvec(aff_right.inverse()[:-1, :-1], disp)
                disp *= affutils.voxel_size(fixed.affine)
                lam = None
                ...
                if moving.previewed:
                    ...
                    preview = warped
                init = affutils.affine_lmdiv(moving.affine, fixed.affine)
                if _almost_identity(init) and moving.shape == fixed.shape:
                    init = moving.dat
                else:
                    init = affine_grid(init, fixed.shape)
            ...
            if grad or hess:
                ...
                g, h, mugrad = self.nonlin.propagate_grad(
                    ...
                    inv=loss.backward)
                mugrad = mugrad.transpose(-1, -2)
                g = batchmatvec(mugrad, g)
                h = sym_matmul(mugrad, h)
                ...
                if is_svf:
                    ...
                    g, h = svfutils.exp_backward(
                        vel00, g, h, steps=self.nonlin.steps)
    ...
    def do_affine(self, logaff, grad=False, hess=False, in_line_search=False):
        ...
        for loss in self.losses:
            ...
            if aff_pos in 'fs':
                ...
                gaff_right = lmdiv(self.nonlin.affine, gaff_right)
                aff_right = aff00 @ aff_right
            aff_right = lmdiv(self.nonlin.affine, aff_right)
            aff_left, gaff_left = self.nonlin.affine, None
            if aff_pos in 'ms':
                gaff_left = gaff00 @ aff_left
                gaff_left = lmdiv(moving.affine, gaff_left)
                aff_left = aff00 @ aff_left
            aff_left = lmdiv(moving.affine, aff_left)
            ...
            if _almost_identity(aff_left) and moving.shape == self.nonlin.shape:
                ...
                left = affine_grid(aff_left, self.nonlin.shape)
                phi = affutils.affine_matvec(aff_left, phi)
            ...
            if grad or hess:
                ...
                if aff_pos in 'fs':
                    ...
                    jac = flow_jacobian(phi0, right)
                    jac = torch.matmul(aff_left[:-1, :-1], jac)
                    mugrad = matvec(jac.transpose(-1, -2), mugrad)
    ...
    def do_affine_only(self, logaff, grad=False, hess=False, in_line_search=False):
        ...
        for loss in self.losses:
            ...
            aff = lmdiv(moving.affine, aff)
            gaff = gaff00 @ fixed.affine
            gaff = lmdiv(moving.affine, gaff)
            phi = affine_grid(aff, fixed.shape)
            ...
            def compose_grad(g, h, g_mu, g_aff):
                ...
                # use deal with this case correctly.
                dim = g_mu.shape[-1]
                g = batchmatvec(g_mu.movedim(0, -1), g.movedim(0, -1))
                h = sym_matmul(g_mu.movedim(0, -2), h.movedim(0, -1))
                g, h = regutils.affine_grid_backward(g, h)
                dim2 = dim * (dim + 1)
                g = g.reshape([*g.shape[:-2], dim2])
                g_aff = g_aff[..., :-1, :]
