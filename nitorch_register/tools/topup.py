...
from nitorch_interpol import (
    add_identity_grid, grid_pull, grid_grad
)
from nitorch_core.finite_differences import diff1d, div1d
from nitorch_core import py, extra
from nitorch_register.optim import FieldCG, OptimIterator
from nitorch_register.losses import make_loss
from nitorch_register.svf1d import exp1d_forward, exp1d_backward
from nitorch_solvers.fields import field_penalty
...
def _deform_1d(img, disp, grad=False):
    img = extra.movedim(img, 0, -2)
    disp = disp.unsqueeze(-1)
    disp = add_identity_grid(disp)
    wrp = grid_pull(img, disp, bound=BND, extrapolate=True)
    wrp = extra.movedim(wrp, -2, 0)
    if not grad:
        return wrp, None
    grd = grid_grad(img, disp, bound=BND, extrapolate=True)
    grd = extra.movedim(grd.squeeze(-1), -2, 0)
    return wrp, grd


def _diff_1d(disp):
    return diff1d(disp, dim=-1, bound=BND, side='c')


def _div_1d(disp):
    return div1d(disp, dim=-1, bound=BND, side='c')


def _exp_1d(vel, model='smalldef'):
    if model == 'svf':
        phi, jac = exp1d_forward(vel, bound=BND, jacobian=True)
        iphi, ijac = exp1d_forward(-vel, bound=BND, jacobian=True)
    else:
        phi, iphi = vel, -vel
        jac = _diff_1d(vel)
        ijac = 1 - jac
...
class TopUpFlexiStep:
    ...
    def __call__(self, vel, grad=False, hess=False, in_line_search=False):
        ...
        if grad:
            ...
            g = extra.movedim(g, 0, -1)
            ig = extra.movedim(ig, 0, -1)
            pos = extra.movedim(pos, 0, -1)
            neg = extra.movedim(neg, 0, -1)
            grad_pos = extra.movedim(grad_pos, 0, -1)
            grad_neg = extra.movedim(grad_neg, 0, -1)

            g0, ig0 = g, ig

            g = fastmath.dot(g0, grad_pos)
            if self.modulation:
                g = g.mul_(jac)
                g0 = fastmath.dot(g0, pos)
                if self.model == 'svf':
                    g0.mul_(jac)
                g += _div_1d(g0)
            ig = fastmath.dot(ig0, grad_neg)
            if self.modulation:
                ig = ig.mul_(ijac)
                ig0 = fastmath.dot(ig0, neg)
                if self.model == 'svf':
                    ig0.mul_(ijac)
                ig += _div_1d(ig0)
            del g0, ig0
            ...
            if hess:
                h = extra.movedim(h, 0, -1)
                ih = extra.movedim(ih, 0, -1)
                h0, ih0 = h, ih

                grad_pos.square_()
                grad_neg.square_()

                h = fastmath.dot(grad_pos, h0)
                if self.modulation:
                    jac.square_()
                    h = h.mul_(jac)
                    h0 = fastmath.dot(pos, h0).square_()
                    if self.model == 'svf':
                        h0.mul_(jac)
                    h += _div_1d(_div_1d(h0))
                ih = fastmath.dot(grad_neg, ih0)
                if self.modulation:
                    ijac.square_()
                    ih = ih.mul_(ijac)
                    ih0 = fastmath.dot(neg, ih0).square_()
                    if self.model == 'svf':
            ...
            if self.model == 'svf':
                g, h = exp1d_backward(vel, g, h, bound=BND)
                ig, ih = exp1d_backward(-vel, ig, ih, bound=BND)
        ...
        if self.verbose and (self.verbose > 1 or not in_line_search):
            ...
            if in_line_search:
                ...
                line = '(topup)  | '
            line += (f'{self.n_iter:03d} | '
                     f'{ll.item():12.6g} + '
                     f'{llv.item():12.6g} = '
                     f'{ll.item() + llv.item():12.6g}')
...
def topup_apply(pos, neg, vel, dim=-1, model='smalldef', modulation=True):
    ...
    if no_batch_pos:
        pos = pos[None]
    pos = extra.movedim(pos, dim, -1)
    no_batch_neg = neg.dim() == ndim
    if no_batch_neg:
        neg = neg[None]
    neg = extra.movedim(neg, dim, -1)
    vel = extra.movedim(vel, dim, -1)
    ...
    pos = extra.movedim(pos, -1, dim)
    neg = extra.movedim(neg, -1, dim)
    if no_batch_pos:
        pos = pos[0]
    if no_batch_neg:
        neg = neg[0]
...
def topup_fit(pos, neg, dim=-1, loss='mse', lam=1, vx=1, ndim=3, mask=None,
    ...
    vx = extra.make_vector(vx, ndim, dtype=pos.dtype).tolist()
    ...
    def regulariser(v):
        return field_penalty(v.unsqueeze(-1), dim=ndim, **{penalty: lam},
                             voxel_size=vx, bound=BND).squeeze(-1)

    step = TopUpFlexiStep(pos, neg, loss, regulariser, verbose,
                          model=model, modulation=modulation, mask=mask)
    optim = FieldCG(factor=lam, voxel_size=vx, **{penalty: 1},
                    bound=BND, max_iter=4)
    optim.search = 'wolfe'
    optim.iter = OptimIterator(max_iter=max_iter, tol=tolerance, stop='diff')
