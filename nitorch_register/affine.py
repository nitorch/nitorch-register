...
from nitorch_fastmath import mdot, matvec, lmdiv, rmdiv, meanm, expm, expm_derivatives
from nitorch_core.py import make_list, majority
from nitorch_core.extra import (
    as_tensor, make_vector, backend as get_backend,
    to_max_backend, max_dtype, max_device,
    unsqueeze, invert_permutation,
)
from nitorch_core.broadcast import expand
from nitorch_core import itertools
import math
...
def volume_axis(*args, **kwargs):
    ...
    * ``def volume_axis(axis, device=None)``
    ...
    def axis_from_name(name, device=None):
        name = name.upper()
        if name == 'R':
            return as_tensor([0, 0], dtype=torch.long, device=device)
        elif name == 'L':
            return as_tensor([0, 1], dtype=torch.long, device=device)
        elif name == 'A':
            return as_tensor([1, 0], dtype=torch.long, device=device)
        elif name == 'P':
            return as_tensor([1, 1], dtype=torch.long, device=device)
        elif name == 'S':
            return as_tensor([2, 0], dtype=torch.long, device=device)
        elif name == 'I':
            return as_tensor([2, 1], dtype=torch.long, device=device)

    def axis_from_index(index, flipped=False, device=None):
        index = as_tensor(index).reshape(())
        flipped = as_tensor(flipped).reshape(())
        return as_tensor([index, flipped], dtype=torch.long, device=device)

    def axis_from_axis(ax, device=None):
        ax = as_tensor(ax, dtype=torch.long, device=device).flatten()
        if ax.numel() != 2:
...
def volume_layout(*args, **kwargs):
    ...
    def layout_from_index(index, flipped=False, device=None):
        index = as_tensor(index, torch.long, device).flatten()
        ndim = index.shape[0]
        flipped = as_tensor(flipped, torch.long, device).flatten()
    ...
    def layout_from_axes(axes, device=None):
        axes = as_tensor(axes, torch.long, device)
...
def layout_matrix(layout, voxel_size=1., shape=0., dtype=None, device=None):
    ...
    # ensure dim
    shape = ensure_shape(shape, [dim], mode='replicate')
    voxel_size = ensure_shape(voxel_size, [dim], mode='replicate')
...
def affine_subbasis(mode, dim=3, sub=None, dtype=None, device=None):
    ...
    if mode == 'T':
        ...
        basis /= torch.sqrt(as_tensor(dim, basis.dtype, device))
        basis[:, dim, dim] = 0
        ...
        k = 0
        isqrt2 = 1 / torch.sqrt(as_tensor(2, basis.dtype, device))
        for i in range(dim):
...
def switch_basis(x, old_basis, new_basis, dim=3, ortho=False):
    ...
    if ortho:
        ...
        old_basis = build_affine_basis(old_basis, dim=dim, **get_backend(x))
        new_basis = build_affine_basis(new_basis, dim=dim, **get_backend(x))

        new_norm = mdot(new_basis, new_basis).sqrt()
        proj = mdot(new_basis[..., :, None, :, :],
                           old_basis[..., None, :, :, :])
        new_x = matvec(proj, x)
...
def _build_affine_basis(basis, dim=None, dtype=None, device=None):
    ...
    dtype = dtype or max_dtype(*built_bases, force_float=True)
    device = device or max_device(*built_bases)
    ...
    for b in basis0:
        if isinstance(b, str):
            ...
            b = as_tensor(b, **info)
        b = unsqueeze(b, dim=0, ndim=max(0, 3-b.dim()))
...
def affine_matrix(prm, *basis, dim=None):
    ...
    *basis : basis_like, default='CSO'
    ...
    def recon(p, B):
        return expm(torch.sum(B*p[..., None, None], dim=-3))
...
def affine_matrix_classic(prm=None, dim=3, *,
    ...
    if prm is not None:
        ...
        prm_t, prm_r, prm_z, prm_s = to_max_backend(prm_t, prm_r, prm_z, prm_s)
        # Broadcast all batch shapes
        batch_shape = expand(prm_t[..., 0], prm_r[..., 0],
                                   prm_z[..., 0], prm_s[..., 0],
                                   shape=[], dry_run=True)
        ...
        def make_affine(t, r, z, sh, o, i):
            if t is not None:
                T = [[i, o, t[0]],
                     [o, i, t[1]],
                     [o, o, i]]
                T = mat_last(as_tensor(T, **backend))
            else:
                T = torch.eye(3, **backend)
                T = expand(T, [*batch_shape, 1, 1])
            if r is not None:
                c = torch.cos(r)
                s = torch.sin(r)
                R = [[c[0],  s[0], o],
                     [-s[0], c[0], o],
                     [o,     o,    i]]
                R = mat_last(as_tensor(R, **backend))
            else:
                R = torch.eye(3, **backend)
                R = expand(R, [*batch_shape, 1, 1])
            if z is not None:
                Z = [[z[0], o,    o],
                     [o,    z[1], o],
                     [o,    o,    i]]
                Z = mat_last(as_tensor(Z, **backend))
            else:
                Z = torch.eye(3, **backend)
                Z = expand(Z, [*batch_shape, 1, 1])
            if sh is not None:
                S = [[i, sh[0], o],
                     [o, i,     o],
                     [o, o,     i]]
                S = mat_last(as_tensor(S, **backend))
            else:
                S = torch.eye(3, **backend)
                S = expand(S, [*batch_shape, 1, 1])
        ...
        def make_affine(t, r, z, sh, o, i):
            ...
            if r is not None:
                ...
                Rz = [[c[2],  s[2], o, o],
                      ...
                      [o,     o,    i, o],
                      [o,     o,    o, i]]
                Rx = mat_last(as_tensor(Rx, **backend))
                Ry = mat_last(as_tensor(Ry, **backend))
                Rz = mat_last(as_tensor(Rz, **backend))
    ...
    zero = expand(zero, batch_shape)
    one = torch.ones([], **backend)
    one = expand(one, batch_shape)
    ...
    def gauss_newton():
        ...
        for n_iter in range(max_iter):
            ...
            for a_basis in basis:
                ...
                M, dM, hM = expm_derivatives(a_prm, a_basis, grad_X=True, hess_X=True)
...
def affine_parameters_classic(mat, return_stacked=True):
    ...
    if dim == 2:
        ...
        cos_zero = (torch.abs(rot_y) - math.pi/2)**2 < 1e-9
...
def affine_matvec(affine, vector):
    ...
    out = matvec(zoom, vector[..., :ndims_in]) + translation
    if is_h:
...
def affine_inv(affine):
    ...
    translation = -matvec(zoom, affine[..., :, -1])
...
def affine_pad(affine, shape, padsize, side=None):
    ...
    backend = get_backend(affine)

    padsize = make_list(padsize, dim, crop=False)
    if side == 'left':
...
def affine_conv(affine, shape, kernel_size, stride=1, padding=0,
    ...
    kernel_size = make_list(kernel_size, ndim)
    stride = make_list(stride, ndim)
    padding = make_list(padding, ndim)
    output_padding = make_list(output_padding, ndim)
    dilation = make_list(dilation, ndim)
...
def affine_default(shape, voxel_size=1., layout=None, center=0.,
    ...
    """
    backend = dict(dtype=dtype or max_dtype(voxel_size, center),
                   device=device or max_device(voxel_size, center))
    shape = make_vector(shape)
    nb_dim = len(shape)
    voxel_size = make_vector(voxel_size, nb_dim, **backend)
    center = make_vector(center, nb_dim, **backend)
    ...
    shift = center - matvec(lin, (shape-1)/2.)
...
def affine_modify(affine, shape, voxel_size=None, layout=None, center=None):
    ...
    backend = get_backend(affine)
    nb_dim = affine.shape[-1] - 1
    shape = make_vector(shape, nb_dim, **backend)
...
def max_bb(all_mat, all_dim, vx=None):
    ...
    dm = torch.solve(mat, torch.cat((mx, torch.ones(1, dtype=dtype, device=device)))[..., None])
...
def affine_reorient(mat, shape_or_tensor=None, layout=None):
    ...
    current_to_ras = invert_permutation(ras_to_current)
...
def mean_space(mats, shapes, voxel_size=None, vx_unit='mm',
    ...
    **fovopt :
    ...
    device = max_device(mats, shapes)
    shapes = as_tensor(shapes, device=device).detach()
    mats = as_tensor(mats, device=device).detach()
...
def compute_fov(mat, affines, shapes, pad=0, pad_unit='%'):
    ...
    affines = as_tensor(affines, **backend)
    shapes = as_tensor(shapes, **backend)
    ...
    if pad is None:
        pad = 0
    pad = make_vector(pad, dim, **backend)
    if pad_unit == '%':
        pad = pad / 100.
        pad = pad * (mx - mn) / 2.
    mx = mx + pad
